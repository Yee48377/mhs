...
export function AppealForm({
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!canSubmit) {
      setError("请先打开具体记录，再从记录详情页进入说明 / 更正。");
      return;
    }
    ...
    try {
      ...
      setMessage((data.message || "说明 / 更正已提交") + " 正在跳转到记录详情...");
      window.setTimeout(() => {
  ...
  return (
    <form className="space-y-5" onSubmit={handleSubmit}>
      ...
      <div className="card-muted px-4 py-4 text-sm leading-7 text-slate-600">
        {initialReportId ? (
          <>
            当前记录：<span className="font-medium text-ink">{initialReportTarget || "这条记录"}</span>。说明 / 更正会附在这条记录下，管理员再决定是否隐藏、撤下或调整展示。
          </>
        ) : (
          <>
            请先打开具体记录，再从详情页进入说明 / 更正，这样不用自己填写记录 ID。
            <div className="mt-3">
      ...
      <div>
        <label className="mb-2 block text-sm font-medium text-slate-700">说明 / 更正</label>
        <textarea
          name="statement"
          className="field min-h-36"
          placeholder="请说明记录哪里需要更正、是否已经解决，或者你希望补充什么情况。"
          required
        />
      </div>
      ...
      <FileUploadField
        label="补充截图（可选）"
        hint="如果有新的聊天记录、退款记录或时间线截图，可以一并上传。"
        multiple
